...
export const projects: ProjectData[] = [
  // ── 1. Haptic Knob ────────────────────────────────────────────────
  {
    ...
    category: 'firmware',
    tags: ['ESP32-S3', 'BLDC', 'FOC', 'SimpleFOC', 'FreeRTOS', 'C++', 'Motor Control'],
    date: '2024',
    status: 'Ongoing',
    ...
    specs: [
      ...
      { label: 'Motor control', value: 'Field-Oriented Control via SimpleFOC v2.3.4' },
      { label: 'Inner control loop', value: '1 kHz (1 ms, vTaskDelayUntil, Core 0)' },
      { label: 'Haptic model rate', value: '200 Hz (every 5th inner-loop cycle)' },
      { label: 'Haptic latency', value: '< 2 ms end-to-end' },
      { label: 'Position sensor', value: 'MT6701 magnetic encoder — SSI via SPI' },
      { label: 'Current sensing', value: 'MCP3204 12-bit SPI ADC — low-side shunts (10.7 mΩ, 50× gain)' },
      { label: 'Supply voltage', value: '5 V — driver limit 3 V, max current 2 A' },
      { label: 'Torque constant', value: '0.035 N·m/A' },
      { label: 'RTOS', value: 'FreeRTOS — ControlTask (P5), TelemetryTask (P2), WatchdogTask (P3)' },
      { label: 'Haptic profiles', value: 'Resistor, Capacitor, Inductor, RLC, Diode' },
      { label: 'Language / toolchain', value: 'C++ — PlatformIO + Arduino framework' },
    ],
    process: [
      {
    ...
    subsystems: [
      {
        label: 'Hardware',
        icon: 'CircuitBoard',
        summary: 'ESP32-S3, MT6701 encoder, MCP3204 ADC, 3-phase gate driver, 5 V supply',
      ...
      },
      {
        ...
        tags: ['Resistor', 'Capacitor', 'Inductor', 'Diode', 'RLC', 'Low-pass Filter'],
      },
      {
        label: 'Testing',
        icon: 'FlaskConical',
        summary: 'Oscilloscope phase verification, haptic latency, profile tuning',
